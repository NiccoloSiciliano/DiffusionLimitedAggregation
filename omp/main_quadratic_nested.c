/*
    ...
    parallelizzata con la libreria omp con il nested parallelism attivato nel
...
int main(int argc, char *argv[])
{
    ...
    // Allocazione della memoria per i mutex per ogni posizione sulla lista delle posizioni successive. 
    omp_lock_t *mutex_grid = calloc((numP), sizeof(omp_lock_t));

    omp_set_nested(1); // Abilitazione del nested parallelism.
	omp_set_max_active_levels(2);   // Impostazione della profondità per il nested parallelism.

    // Inizializzazione dei mutex
    for (int i = 0; i < numP; i++)
        omp_init_lock(mutex_grid + i);
    ...
    {
        #pragma omp parallel for num_threads((int)(floor(sqrt(numT))))  
        for (int p1 = 0; p1 < numP; p1++)   // Ogni particella viene confrontata.
        {
            ...
            // Fase di controllo.
            #pragma omp parallel for num_threads(numT/((int)(floor(sqrt(numT))))) 
            for (int p2 = 0; p2 < numP+1; p2++) // Con tutte le altre particelle.
            {
                ...
                {
                    // Lock sul mutex associato alla posizione successiva della particella p1.
                    omp_set_lock(mutex_grid+p1);
                    if (iter == 0)
                    ...
                    // Unlock sul mutex associato alla posizione successiva della particella p1.
                    omp_unset_lock(mutex_grid+p1);
                }
            }
