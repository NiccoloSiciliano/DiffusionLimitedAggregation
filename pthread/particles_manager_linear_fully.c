/*
    Questo file contiene la funzione eseguita da ogni thread del programma "fully parallel"(pthread/main_linear_fully.c).
*/
...
void particles_manager(void* args)
{
    ...
    barrier_wait(&barrierSpawn, numthread, 0);              // Barriera per sincronizzare i thread e prendere il tempo di inizio.
    if (firstP == 0)                                        // Il tempo è preso dal thread che si occupa della particella 0.
        ...
        for (int p = firstP; p < lastP; p++)
        {
            ...
            switch (actPos[p].stuck)
            {
                ...
                {   
                    ...
                    pthread_mutex_lock(mutext + actPos[p].x + dimG*actPos[p].y + dimG*dimG*actPos[p].z); 
                    ...
                    pthread_mutex_unlock(mutext + actPos[p].x + dimG*actPos[p].y + dimG*dimG*actPos[p].z); 
                    
                    // Lock sul contatore relativo alla cella della griglia nella quale si trovava la particella.
                    pthread_mutex_lock(mutext + prePos[indp].x + dimG*prePos[indp].y + dimG*dimG*prePos[indp].z);
                    int *prev = grid + prePos[indp].x + dimG*prePos[indp].y + dimG*dimG*prePos[indp].z;
                    (*prev)--;  // Decrementa il contatore.
                    // Unlock del contatore.
                    pthread_mutex_unlock(mutext + prePos[indp].x + dimG*prePos[indp].y + dimG*dimG*prePos[indp].z);  
                    break;
                }
