/*
    Questo file contiene l'implementazione partially parallel con Pthread dell'algoritmo lineare.
    Per "partially parallel" si intende che tutte le fasi di computazione delle particelle sono eseguite in parallelo
    tranne la fase di movimento.
*/
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_t *mutext = calloc(1, sizeof(pthread_mutex_t));   // Allocazione della memoria per il mutex per l'accesso alla fase di spostamento
    ...
    *(grid + seed.x + dimG*seed.y + dimG*dimG*seed.z) = -1; // Contatore posto a -1 per segnalare la presenza di un cristallo.
    ...
    for (int t = 0; t < numthread; t++)
    {
        ...
        // Avvio dei thread che eseguono la funzione particles_manager (dichiarata in pthread/particle_manager_linear_partial.c)
