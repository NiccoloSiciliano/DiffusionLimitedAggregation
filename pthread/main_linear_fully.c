/*
    Questo file contiene l'implementazione fully parallel con Pthread dell'algoritmo lineare.
...
int main(int argc, char *argv[])
{
    ...
    Particle *actPos = calloc(sizeof(Particle),numP+1); // Allocazione della memoria per la lista di particelle (Particel definita in util/particleLogic.h).
    ...
    int *grid = calloc(dimG*dimG*((dimG-1)*threed + 1), sizeof(int));   // Allocazione della memoria per la griglia di contatori.
    pthread_mutex_t *mutext = calloc(dimG*dimG*((dimG-1)*threed +1), sizeof(pthread_mutex_t));  // Allocazione della memoria per i mutex 
                                                                                                    //necessari per l'accesso in scrittura alla griglia di contatori.
    // Inizializzazione della griglia di mutex.
    for (int i = 0; i < dimG; i++)
        for (int j = 0; j < dimG; j++)
            for (int z = 0; z < ((dimG-1)*threed +1); z++)
                pthread_mutex_init(mutext + i + dimG*j + dimG*dimG*z, NULL);

    // Il seme scelto dall'utente viene inserito alla fine della lista di particelle.
    actPos[numP] = seed;
    *(grid + seed.x + dimG*seed.y + dimG*dimG*seed.z) = -1; // Contatore posto a -1 per segnalare la presenza di un cristallo.
    ...
    for (int t = 0; t < numthread; t++)
    {
        ...
        targs[t].grid = grid;
        targs[t].mutext = mutext;
        // Avvio dei thread che eseguono la funzione particles_manager (dichiarata in pthread/particle_manager_linear_fully.c)
    ...
    for (int i = 0; i < dimG; i++)
            for (int j = 0; j < dimG; j++)
                for (int z = 0; z < ((dimG-1)*threed +1); z++)
                    pthread_mutex_destroy(mutext + i + dimG*j + dimG*dimG*z);
