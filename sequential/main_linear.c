/*
    Questo file contiene l'implementazione sequenziale dell'algoritmo lineare.
*/
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
{
    int dimG, numP, numI;   // Dimensione della griglia, numero di particelle, numero di iterazioni.
    ...
    actPos[numP] = seed;
    // Il contatore relativo alla posizione del cristallo iniziale viene impostato a -1.
    *(grid + seed.x + dimG*seed.y + dimG*dimG*seed.z) = -1;
    ...
    for (int iter = 0; iter < numI; iter++) 
    {
        ...
        for (int p = 0; p < numP; p++)
        {
            if (actPos[p].stuck == 0) // Se la particella non è cristallizzata.
            {
                // Contatore della griglia relativo all'attuale posizione della particella.
                int *v = grid + actPos[p].x + dimG*actPos[p].y + dimG*dimG*actPos[p].z;
                if (*v >= 0)    // Se non c'è un cristallo sulla cella.
                    (*v)++;     // Incrementa il contatore.
                // Contatore della griglia relativo alla posizione precedente della particella.
                int *prev = grid + prePos[p].x + dimG*prePos[p].y + dimG*dimG*prePos[p].z;
                (*prev)--;  // Decrementa il contatore.
            }
            if (actPos[p].stuck == 1) // Se la particella si sta cristallizzando.
            {
                int *v = grid + actPos[p].x + dimG*actPos[p].y + dimG*dimG*actPos[p].z;
